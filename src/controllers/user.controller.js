...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  const channel = await User.aggregate([
    ...
    {
      $project: {
        //used to provide select values to the user  by making flag as 1 or true
        fullName: 1,
        username: 1,
        subscribersCount: 1,
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId(req.user._id), //here the way of accessing id is different bcoz in monodb id is in string so when we access normally mongoose behind the scene directly give us id but in aggregate as it is object comes with string id so we need to convert it
      },
    },
    {
      $lookup: {
